...
EventCut::EventCut(TTree* tree)
{
    ...
    mVzCut[0] = -999;
    mVzCut[1] = 999;
    mVzVpdVzDiffCut[0] = -999;
    mVzVpdVzDiffCut[1] = 999;
    mVrCut = 999;
...
TEventList* EventCut::makeEventList()
{
    ...
    mTree->SetBranchStatus("MuEvent.mVpdVz*",1);
...
//    nTotal = 1000;
    for(Int_t i = 0; i <= (nTotal-1); i++)
    {
        ...
        mVzVpdVzDiff = mTree->GetLeaf("MuEvent.mVpdVz")->GetValue() - mTree->GetLeaf("PrimaryVertices.mPosition.mX3")->GetValue();
        mRefmult = mTree->GetLeaf("MuEvent.mRefMultNeg")->GetValue() + mTree->GetLeaf("MuEvent.mRefMultPos")->GetValue();

        mVr = sqrt(mVx*mVx + mVy*mVy);

        mVzPass = checkVz();
        mVzVpdVzDiffPass = checkVzVpdVzDiff();
        mVrPass = checkVr();
        mHasVertexPass = checkHasVertex();
        mTriggerPass = checkTrigger();
	cout<<"vz\t"<<mVzPass
	    <<"\tVr\t"<<mVrPass
	    <<"\tVzDiff\t"<<mVzVpdVzDiffPass
	    <<"\tvertex\t"<<mHasVertexPass
	    <<"\ttrigger\t"<<mTriggerPass<<endl;
	    
//        mEventPass = mVzPass && mVzVpdVzDiffPass && mHasVertexPass && mTriggerPass;
//        mEventPass = mVzPass && mVrPass && mHasVertexPass && mTriggerPass;
        mEventPass = mVzPass && mHasVertexPass && mTriggerPass;
        fillHistograms();
