...
public:
    ...
    void SetVzVpdVzDiffCut(const Float_t VzVpdVzDiffLow, const Float_t VzVpdVzDiffHigh) {mVzVpdVzDiffCut[0] = VzVpdVzDiffLow; mVzVpdVzDiffCut[1] = VzVpdVzDiffHigh;}
...
private:
    ...
    Bool_t checkVzVpdVzDiff() {return (mVzVpdVzDiff >= mVzVpdVzDiffCut[0]) && (mVzVpdVzDiff <=mVzVpdVzDiffCut[1]);}
    Bool_t checkRefMult() { return (mRefmult >= mRefmultCut[0]) && (mRefmult <= mRefmultCut[1]); }
    Bool_t checkVr() { return (mVr <= mVrCut); }
    Bool_t checkHasVertex() { return ( fabs(mVx)>=1e-9 && fabs(mVy)>=1e-9 && fabs(mVz)>=1e-9 ); }
    Bool_t checkZdc() { return (mZdc[0] <= mZdcCut) && ( mZdcCut <=mZdc[1]); }
    Bool_t checkTrigger();
