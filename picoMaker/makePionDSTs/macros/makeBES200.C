...
void makeBES200(const TString fileList = "test200.list"
				...
				)
{
    ...
    Float_t VzCut = 30.;                 // Cut on z-vertex
    Float_t VzVpdVzDiffCut =5.; 
    Float_t VrCut = 200.;                // Cut on r-vertex
    Float_t VrCenter[2] = {0.,0.};        // Beam center - (0, -0.89) for 14.6 GeV
    Float_t etaGap = 10;
   Float_t triggers[5] = {350003, 350013, 350023, 350033, 350043}; 
   Int_t nTriggers = 5;
...
//	StFlowCutEvent::SetVertexZ(-1*VzCut, VzCut);
    ...
    eventCut->SetVzVpdVzDiffCut(-1.0*VzVpdVzDiffCut, VzVpdVzDiffCut);
    eventCut->SetVrCut(VrCut);
    eventCut->SetZdcCut(zdcCut);
